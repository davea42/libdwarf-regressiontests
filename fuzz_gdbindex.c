...
int main(int argc, char **argv)
{
  ...
  if (fd < -0) {
      printf("fuzz_gdbindex failed to open %s\n",filename);
  }
  res = dwarf_init_b(fd, DW_GROUPNUMBER_ANY, errhand,
    errarg, &dbg, &error);

  if (res != DW_DLV_OK) {
    if (res == DW_DLV_ERROR) {
      printf("ERROR calling dwarf_init_b %s\n",
        dwarf_errmsg(error));
      dwarf_dealloc_error(dbg, error);
    }
  } else {
    res = examplew(dbg, &error);
    if (res == DW_DLV_ERROR) {
      printf("ERROR calling examplew %s\n",
        dwarf_errmsg(error));
      dwarf_dealloc_error(dbg, error);
    } else if (res == DW_DLV_OK) {
      printf("No error\n");
    } else {
      printf("got DW_DLV_NO_ENTRY\n");
    }
  }
  dwarf_finish(dbg);
  close(fd);
